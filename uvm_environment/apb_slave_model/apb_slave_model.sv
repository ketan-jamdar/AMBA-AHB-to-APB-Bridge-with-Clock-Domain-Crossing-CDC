    ...
    always_ff @(posedge vif.PCLK or negedge vif.PRESETn) begin
        if (!vif.PRESETn) begin
           ...
           // end
            ...
            // Accept a NEW transfer only on the cycle PENABLE first
            // rises with PSEL asserted -- not merely whenever
            // PSEL&&PENABLE&&PREADY happen to be simultaneously true,
            // which also occurs on this transfer's own completion
            // cycle and was causing a spurious second accept.
