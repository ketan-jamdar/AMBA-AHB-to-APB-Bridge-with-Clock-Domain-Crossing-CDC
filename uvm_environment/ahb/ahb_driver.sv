...
class ahb_driver extends uvm_driver #(ahb_transaction);
    ...
    task drive_transfer(ahb_transaction tr);
        ...
        vif.drv_cb.HADDR  <= tr.haddr;
         ...
         $display("[%0t] DRIVER SEND ADDR=%h DATA=%h WRITE=%0b",
         $time,
         tr.haddr,
         tr.hwdata,
         tr.hwrite);
        ...
        if(tr.hwrite) begin
         ...
         // Wait until DUT completes transfer
         while(!vif.HREADYOUT)
        @(posedge vif.HCLK);
        ...
        // Wait until bridge pulls HREADYOUT LOW
            @(posedge vif.HCLK);
        ...
        `uvm_info(get_type_name(),
                  $sformatf("Transfer Completed : %s ADDR=%08h",
