...
always_ff @(posedge pclk or negedge presetn) begin
    if (!presetn) begin
        ...
        // Debug Print
        $display("[%0t] PCLK capture %h", $time, read_data);
...
//////////////////////////////////////////////////////////////
// Data/error capture: read_data_reg/error_reg are quasi-static
// (guaranteed not to change until busy_pclk deasserts, which can
// only happen AFTER the ack round-trips), so a direct,
// single register-to-register sample here is fully coherent.
//////////////////////////////////////////////////////////////
always_ff @(posedge hclk or negedge hresetn) begin
    if (!hresetn) begin
        ...
        if (req_pulse_hclk) begin
            ...
            // Debug Print
            $display("[%0t] HCLK capture %h", $time, read_data_reg);
        end
    end
end

//////////////////////////////////////////////////////////////
// ACK generation: fires one cycle AFTER data has actually been
// latched into prdata_sync/pslverr_sync (i.e. when read_valid_sync
// itself asserts) -- ACK means "data consumed," not "request seen."
//////////////////////////////////////////////////////////////
always_ff @(posedge hclk or negedge hresetn) begin
    if (!hresetn)
        ack_toggle <= 1'b0;
